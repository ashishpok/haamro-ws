...
Ext.define('Ext.ux.TouchCalendarView', {
  ...
  requires: [
   	...
   	commonTemplateFunctions: {
	    ...
	    getTimeSlotDateTpl: function(){
   		...
   		getClasses: function(values){
   			...
   			if(values.weekend){
	          if (!values.prevMonth && !values.nextMonth && (this.me.getViewMode() == 'NEPALI')) {
	            classes.push('nepali-cal-weekend');
	          } else if (this.me.getViewMode() == 'NEPALI') {
	            // do nothing
	          } else {
	            classes.push(this.me.getWeekendCls());
	          }
   			} else if (!values.prevMonth && !values.nextMonth && (this.me.getViewMode() == 'NEPALI') && (NepaliCalendarUtil.isHoliday(values.date))) {
   				classes.push('nepali-cal-weekend');
   			}
	    ...
	    isEndOfPeriod: function(currentIndex){
          ...
          //if (this.getViewMode() != 'NEPALI') {
   				   daysArray.push(values[i]);
          //} 

          //else {
          //  daysArray.push(NepaliCalendarUtil.getNepaliWeekDays()[i]);
          //}
   			}
   		...
   		getHeaderClass: function(currentIndex){
   			var ret = currentIndex === 1 ? this.me.getPrevPeriodCls() : currentIndex === 7 ? this.me.getNextPeriodCls() : '';
        return ret;
   		}
   	},
			...
			Ext.define('TouchCalendarViewModel', {
                ...
                config: {
                    fields: [
                        ...
                        {name: 'nepaliMonthStartDate', type: 'date', defaultValue: null},
                        {name: 'nepaliMonthEndDate', type: 'date', defaultValue: null},
                        {name: 'nepaliMonth', type: 'string', defaultValue: 0},
                        {name: 'nepaliYear', type: 'string', defaultValue: 0},
                        {name: 'nepaliDate', type: 'string', defaultValue: 0},
                        'timeSlots'
                    ]
                }
			});
	...
	populateStore: function(){
      ...
      baseNepaliVals = NepaliCalendarUtil.getNepaliMonthValuesInUTC(baseDate),

            record;
		
    //console.log("populateStore - iterDate: " + iterDate + " ,totalDays: " + totalDays);		
      ...
      iterNepaliVals = NepaliCalendarUtil.getNepaliMonthValuesInUTC(iterDate);
      ...
      if (this.getViewMode() != 'NEPALI') {
        ...
        prevMonth = iterNepaliVals.month < baseNepaliVals.month;
        nextMonth = iterNepaliVals.month > baseNepaliVals.month;
      }
      ...
      record = Ext.create('TouchCalendarViewModel', {
          ...
          date: iterDate,
          nepaliMonthStartDate: baseNepaliVals.startDate,
          nepaliMonthEndDate: baseNepaliVals.endDate,
          nepaliYear: baseNepaliVals.year,
          nepaliMonth: baseNepaliVals.monthName,
          nepaliDate: baseNepaliVals.nepaliDate,
      });
	...
	statics: {
      ...
      NEPALI: {
        ...
        /**
           * Template for the NEPALI MONTH view
           */
          tpl: [
                      '<table class="{[this.me.getViewMode().toLowerCase()]}">',
                        '<thead>',
                            '<tr>',
                                '<tpl for="this.getDaysArray(values)">',
                                    ...
                                    '</tpl>', 
                                    '<tpl if="xindex === 2">',
                                      '<th colspan="5" class="{[this.getHeaderClass(xindex)]}">',
                                        '<div class="month-header">',
                                          '<div class="nepali-month">{nepaliMonth} {[NepaliCalendarUtil.convertToNepaliNumber(values.nepaliYear)]}</div>',
                                          '<div class="utc-month">({[Ext.Date.format(values.nepaliMonthStartDate, "M")]}/{[Ext.Date.format(values.nepaliMonthEndDate, "M")]} {[Ext.Date.format(values.nepaliMonthEndDate, "Y")]})</div>',
                                        '</div>',
                                      '</th>',
                                    '</tpl>', 
                                    '<tpl if="xindex === 7">',
                        ...
                        '</thead>',
                        '<tbody>',
                            '<tr class="x-haamro-cal-days-row">',
                                '<tpl for="this.getDaysArray(values)">',
                                    '<th class="{[this.getHeaderClass(xindex)]} x-haamro-cal-days-cell">',
                                      '{[NepaliCalendarUtil.getNepaliDayOfWeek(values.date)]}',
                                    '</th>',
                                '</tpl>',
                            '</tr>',
                            '<tr class="time-block-row">',
                            '<tpl for=".">',
                                ...
                                '<td class="time-block {[this.getClasses(values)]}" datetime="{[this.me.getDateAttribute(values.date)]}">',
                                   '<div class="x-haamro-container">',
		                           		'<div class="x-haamro-calendar-day-header">{[NepaliCalendarUtil.getFirstParva(values.date)]}</div>',
		                        		'<div class="x-haamro-calendar-day-content">',
		                        			'<div class="x-haamro-calendar-day-side1">',
		                        				'<div class="x-haamro-calendar-day-rotate">{[Ext.os.is("Phone") ? "" : NepaliCalendarUtil.getSecondParva(values.date)]}</div>',
		                        			'</div>',
		                        			'<div class="x-haamro-calendar-day-main">{[NepaliCalendarUtil.getNepaliDate(values.date)]}</div>',
		                        			'<div class="x-haamro-calendar-day-side2">{[Ext.Date.format(values.date, "j")]}</div>',
		                        		'</div>',
		                        		'<div class="x-haamro-calendar-day-footer">{[NepaliCalendarUtil.getNepaliTithi(values.date)]}</div>',
                                   '</div>',
                                '</td>',
         ...
         */
        getNextIterationDate: function(d, index){
          //console.log("getNextIterationDate - d: " + d + ", index: " + index);
          return new Date(d.getFullYear(), d.getMonth(), d.getDate() + (index===0?0:1));
        },
         ...
         */
        getTotalDays: function(date){
          //console.log("getTotalDays - date: " + date);
          var nepaliMonth = NepaliCalendarUtil.getNepaliMonthValuesInUTC(date);
          //var firstDate = Ext.Date.getFirstDateOfMonth(date);
          var random1 = NepaliCalendarUtil.getNepaliMonthValuesInUTC(Ext.Date.add(new Date(), Ext.Date.DAY, 16));
          var random2 = NepaliCalendarUtil.getNepaliMonthValuesInUTC(Ext.Date.add(new Date(), Ext.Date.DAY, 17));
          var random3 = NepaliCalendarUtil.getNepaliMonthValuesInUTC(Ext.Date.add(new Date(), Ext.Date.DAY, 18));

          //console.log("RANDOM :- " + random1.month + ',' + random1.nepaliDate + ',' + random1.diff + ' : ' 
          //                        + random2.month + ',' + random2.nepaliDate + ',' + random2.diff + ' : ' 
          //                        + random3.month + ',' + random3.nepaliDate + ',' + random3.diff);

          //return this.isWeekend(nepaliMonth.startDate) ? 42 : 35;
          return 42;
        },
         ...
         */
        getStartDate: function(date){
          //console.log(nepaliMonth.startDate + ":" + nepaliMonth.endDate + ":" + nepaliMonth.days + ":" + nepaliMonth.month);
          //console.log("getStartDate - date: " + date);
          var nepaliMonth = NepaliCalendarUtil.getNepaliMonthValuesInUTC(date);
          //return Ext.bind(Ext.ux.TouchCalendarView.WEEK.getStartDate, this)(new Date(date.getFullYear(), date.getMonth(), 1));
          return Ext.bind(Ext.ux.TouchCalendarView.WEEK.getStartDate, this)(new Date(nepaliMonth.startDate.getFullYear(), nepaliMonth.startDate.getMonth(), nepaliMonth.startDate.getDate()));
        },
         ...
         */
        getDeltaDate: function(date, delta){
          //console.log("getDeltaDate - date: " + delta);
          var newMonth = date.getMonth() + delta,
